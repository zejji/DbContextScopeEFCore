...
/// <summary>
/// Convenience methods to create a new ambient <see cref="IDbContextScope"/>. This is the preferred method
/// to create a <see cref="IDbContextScope"/>.
/// </summary>
public interface IDbContextScopeFactory
{
    /// <summary>
    /// Creates a new <see cref="IDbContextScope"/>.
    ///
    ...
    /// <summary>
    /// Creates a new <see cref="IDbContextScope"/> for read-only queries.
    ///
    /// By default, the new scope will join the existing ambient scope. This
    /// is what you want in most cases. This ensures that the same "DbContext" instances
    /// are used by all services methods called within the scope of a business transaction.
    ...
    /// <summary>
    /// Forces the creation of a new ambient <see cref="IDbContextScope"/> (i.e. does not
    ...
    /// <summary>
    /// Forces the creation of a new ambient read-only <see cref="IDbContextScope"/> (i.e. does not
    /// join the ambient scope if there is one) and wraps all "DbContext" instances
    /// created within that scope in an explicit database transaction with
    /// the provided isolation level.
    ///
    ...
    /// and only if you fully understand the implications of doing this.
    /// </summary>
    IDisposable CreateReadOnlyWithTransaction(IsolationLevel isolationLevel);

    /// <summary>
    /// Temporarily suppresses the ambient <see cref="IDbContextScope"/>.
    ///
    /// Always use this if you need to kick off parallel tasks within a <see cref="IDbContextScope"/>.
    /// This will prevent the parallel tasks from using the current ambient scope. If you
    /// were to kick off parallel tasks within a <see cref="IDbContextScope"/> without suppressing the ambient
    /// context first, all the parallel tasks would end up using the same ambient <see cref="IDbContextScope"/>, which
